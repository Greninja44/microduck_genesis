...
| base z | uniform 0.12–0.13 m | uniform 0.12–0.13 m | MATCHED |
| base x/y/yaw | x/y ±0.5 m, yaw ±pi | same per environment | MATCHED |
...
The 24-step rollout spans 0.48 s and therefore does not directly expose the
~1 s deterministic failure trajectory within one rollout. It matches upstream
and is intentionally unchanged until all other environment parity is closed.
