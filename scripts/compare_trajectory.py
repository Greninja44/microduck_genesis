...
def main():
 p=argparse.ArgumentParser(); p.add_argument('--genesis',default='logs/genesis_replay.npz'); p.add_argument('--output',default='logs/trajectory_parity.csv'); a=p.parse_args()
 m=np.load('logs/mujoco_reference.npz'); g=np.load(a.genesis); Path('logs').mkdir(exist_ok=True)
