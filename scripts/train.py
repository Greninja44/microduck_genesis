...
def main():
 ...
 for it in range(a.iterations):
  ...
  for _,_,_,val,rew,done in reversed(rollout): gae=rew+.99*last*(~done)-val+.99*.95*(~done)*gae; returns.append(gae+val); last=val
  returns=torch.cat(list(reversed(returns))); O,A,LP,V,R=map(torch.cat,zip(*[(x[0],x[1],x[2],x[3],r) for x,r in zip(rollout,reversed(returns))]))
  ...
  print(f'iteration={it} fps={a.num_envs*a.steps_per_env/elapsed:.1f} mean_reward={torch.stack([x[4] for x in rollout]).mean().item():.3f} policy_loss={mean[0]:.4f} value_loss={mean[1]:.4f} entropy={mean[2]:.3f} vram_mib={mem:.0f}')
