...
def main():
    ...
    fields=['iteration','mean_reward','episode_length','termination_rate','mean_vx','tracking_rmse','mean_abs_action','max_abs_action','action_std','mean_abs_bam_torque','max_abs_bam_torque','torque_saturation_fraction','policy_loss','value_loss','entropy','gradient_norm','actor_parameter_norm','critic_parameter_norm','log_std_min','log_std_max','ppo_fps','vram_mib','ram_mib']; new=not mp.exists() or mp.stat().st_size==0; mf=mp.open('a',newline=''); writer=csv.DictWriter(mf,fieldnames=fields); writer.writeheader() if new else None
    obs=env.reset()
    for local in range(a.iterations):
        iteration=start+local+1; env.rewarder.set_training_steps((iteration-1)*a.steps_per_env); rollout=[]; started=time.perf_counter(); av=[]; tv=[]; vv=[]; dv=[]
        for _ in range(a.steps_per_env):
            ...
            finite('sampled action',act); nxt,rew,done,info=env.step(act); finite('reward',rew); finite('next observation',nxt); finite('critic observation',info['critic_obs'])
            av.append(act.detach()); tv.append(env.last_bam_torque.detach().clone()); _,_,_,_,vel,_=env._state(); finite('base state',vel); vv.append(vel[:,0].detach()); dv.append(done.detach()); rollout.append((obs,env.critic_obs.clone(),act,lp,val,rew,done)); obs=nxt
